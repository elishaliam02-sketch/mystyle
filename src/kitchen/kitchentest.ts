 ...
 */
import { dailyTarget, dietOk, dietHidden, foodDietOk, plateForGoal, searchFoods, shoppingList, goalFit, commonsSearchUrl, pickPhoto, photoQueries, readPantry, readPantryFull, suggestMeals, slotForHour, starterMeals, yourPlate } from "./index";
...
// --- the picker keeps photographs and rejects everything else
{
  const page = (over: Partial<CommonsPage> & { info?: Record<string, unknown> }): CommonsPage => ({
    ...
    index: 1,
    imageinfo: [{ thumburl: "https://upload.wikimedia.org/x.jpg", mime: "image/jpeg", width: 2000, height: 1400, ...over.info }],
    ...over,
  });

  check("a plain photograph is taken", pickPhoto([page({})]) === "https://upload.wikimedia.org/x.jpg");
  ...
  check("the best-ranked photo wins", ranked === "https://upload.wikimedia.org/first.jpg");
  ...
  check("a landscape frame breaks a tie", tall === "https://upload.wikimedia.org/wide.jpg");
  ...
  check("junk is skipped rather than returned", skipped === "https://upload.wikimedia.org/real.jpg");
}
